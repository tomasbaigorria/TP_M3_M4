...
```
    ...
    - El gráfico da cuenta de una relación difusa entre las dos variables. Algo similar ocurre con la edad:
...
```
    ...
    - En el caso de la edad, vemos que la distribución de los datos se asemeja a una campana de gauss. Esto se vincula naturalmente con la ocurrencia de menores ingresos pasada la edad jubilatoria. Así lo visualizamos en el siguiente gráfico:
...
wflow1 <- workflow() %>% 
  add_model(lmodel) %>% 
  add_formula(INGRESO_OCUP_PRINCIPAL ~ NIVEL_ED)  
                                                                                 #mostrar con métricas
